# Описание

*Portal Explorer* — бесплатная веб-программа для исследования всех видов порталов. Portal Explorer использует рей-трейсинг внутри шейдеров для рисования сцен. Так же, в нём имеется редактор сцен в реальном времени, без перекомпиляции программы! Вы тоже можете создавать свои сцены и вместе исследовать порталы.

---

*Версия:* 0.2.0
*Автор:* Илья Шепрут
*Исхоники:* <https://github.com/optozorax/portal>
*Лицензия:* MIT или Apache 2.0

---

# Автор

[Канал в телеграме](https://t.me/optozorax_dev) — главное место где я пишу, на русском.
[Твиттер](https://twitter.com/optozorax) — иногда пишу здесь на английском.
[GitHub](https://github.com/optozorax)
[YouTube](https://www.youtube.com/channel/UCIsVwdLhYrrs3K8MOXEZdhQ)
[Блог](https://optozorax.github.io)

---

# Сделано с помощью

Язык программирования: [Rust](https://www.rust-lang.org)
Движок: [macroquad](https://github.com/not-fl3/macroquad)
Интерфейс: [egui](https://github.com/emilk/egui)
Формулы: [fasteval](https://github.com/likebike/fasteval)

---

# Ссылки на использованный код

Рендеринг капсулы: [@shadertoy](https://www.shadertoy.com/view/Xt3SzX)
Проекция Панини: [@shadertoy](https://www.shadertoy.com/view/Wt3fzB)